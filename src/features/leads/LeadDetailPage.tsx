  ...
  return (
    <div>
      ...
      <div className="mb-6">
        ...
        {/* Acciones — apiladas en móvil, en fila en desktop */}
        <div className="flex flex-wrap gap-2">
          {isNew && (
            <button
              onClick={handleSetInReview}
              disabled={updating}
              className="flex-1 sm:flex-none px-4 py-2.5 text-sm font-medium text-slate-700 bg-white border border-slate-200 rounded-lg hover:bg-slate-50 transition-colors disabled:opacity-50"
            >
              Iniciar revisión
            </button>
          )}
          {canAccept && (
            <button
              ...
              disabled={updating}
              className="flex-1 sm:flex-none inline-flex items-center justify-center gap-2 px-4 py-2.5 text-sm font-medium text-white bg-green-600 rounded-lg hover:bg-green-700 transition-colors disabled:opacity-50"
            >
          ...
          {canConvert && (
            <button
              ...
              className="flex-1 sm:flex-none inline-flex items-center justify-center gap-2 px-4 py-2.5 text-sm font-medium text-white bg-slate-900 rounded-lg hover:bg-slate-800 transition-colors"
            >
          ...
          {canReject && (
            <button
              ...
              disabled={updating}
              className="flex-1 sm:flex-none inline-flex items-center justify-center gap-2 px-4 py-2.5 text-sm font-medium text-red-600 bg-white border border-red-200 rounded-lg hover:bg-red-50 transition-colors disabled:opacity-50"
            >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        {/* Columna principal — ocupa 2 cols en desktop */}
          ...
          <div className="bg-white border border-slate-200 rounded-xl p-5">
            ...
            <div className="space-y-3">
              <div>
                ...
                <p className="text-sm text-slate-700">
                  {format(lead.createdAt, "dd 'de' MMMM 'de' yyyy", { locale: es })}
                </p>
              </div>
              <div>
                <p className="text-xs text-slate-500">Última actualización</p>
                <p className="text-sm text-slate-700">
                  {format(lead.updatedAt, "dd 'de' MMMM 'de' yyyy", { locale: es })}
                </p>
              </div>
            </div>
          </div>
