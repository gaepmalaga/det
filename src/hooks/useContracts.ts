...
export function useCollaboratorContracts(collaboratorId: string) {
  const { user } = useAuth()
  const [contracts, setContracts] = useState<Contract[]>([])
  const [loading, setLoading] = useState(true)
  const [error, setError] = useState<string | null>(null)
  ...
  const load = useCallback(async () => {
    ...
    setError(null)
    try {
      const data = await getContractsByCollaborator(firmId, collaboratorId)
      setContracts(data)
    } catch (err) {
      console.error(err)
      setError('Error al cargar los contratos.')
